...
class ShopperAgentClient {
  ...
  async query(userMessage) {
    try {
      ...
      console.log(`Retrieved shopper agent: ${agent.name}`);
      ...
      if (run.status === "failed") {
        console.error(`Shopper agent run failed:`, run.lastError);
        return "Sorry, I encountered an error processing your shopping request.";
      }

      console.log(`Shopper agent run completed with status: ${run.status}`);
      ...
      return assistantResponse || "No response from shopper agent.";
    } catch (error) {
      console.error("Error in shopper agent client:", error);
      throw error;
    }
  }
}

module.exports = { ShopperAgentClient };
